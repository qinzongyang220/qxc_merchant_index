...
object HiveProductAnalysisQueryJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      // 获取要分析的日期：如果传入参数则使用参数，否则使用昨天
      val analyzeDate = if (args.length > 0) {
        // 验证日期格式
        try {
          ...
          case e: Exception =>
            println(s"错误：日期格式不正确，请使用 yyyy-MM-dd 格式，例如：2025-01-20")
            throw e
        }
      } else {
        // 默认使用昨天
      ...
      println(s"分析日期: $analyzeDate")
      // 处理指定日期的数据
      processDateData(spark, analyzeDate)
   ...
   * 处理指定日期的数据
   */
  private def processDateData(spark: SparkSession, currentDate: String): Unit = {
    try {
      spark.sql("USE mall_bbc")
      // 执行商品分析主查询
      val productAnalysisSQL = s"""
      ...
      order_data AS (
        ...
        WHERE o.create_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
          AND oi.rec_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
      ...
      pay_data AS (
        SELECT
          ...
          SUM(CAST(oi.prod_count AS INT)) AS pay_num,  -- 新增：支付件数
          SUM(CAST(oi.actual_total AS DECIMAL(18,2))) AS pay_amount
        FROM mall_bbc.t_ods_tz_order o
        JOIN mall_bbc.t_ods_tz_order_item oi ON o.order_number = oi.order_number
        WHERE o.is_payed = 'true'
          AND o.pay_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
          AND oi.rec_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
        GROUP BY oi.prod_id, o.shop_id
      ),
      refund_single AS (
        ...
        WHERE r.apply_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
          AND r.refund_type = '2'
          AND oi.rec_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
      ...
      refund_order AS (
        ...
        WHERE r.apply_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
          AND r.refund_type = '1'
          AND o.create_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
          AND oi.rec_time BETWEEN '$currentDate 00:00:00' AND '$currentDate 23:59:59'
          AND oi.prod_id IS NOT NULL
          AND oi.shop_id IS NOT NULL
      ),
      ...
      SELECT
        ...
        COALESCE(pd.pay_num, 0) AS pay_num,  -- 新增：支付件数
        ...
        p.status AS status,
        TO_DATE('$currentDate') AS stat_date
      ...
      // 显示数据内容
      if (dedupCount > 0) {
        println(s"\n======= 商品分析数据 (共 $dedupCount 条) =======")
        dedupDF.show(50, false)

        // 写入MySQL
        println(s"\n准备写入 $dedupCount 条数据到MySQL表: tz_bd_merchant_product_analysis")
        writeToMySQL(dedupDF, "tz_bd_merchant_product_analysis", currentDate)
        println(s"数据写入完成 (日期: $currentDate)")
      } else {
        println("没有数据需要写入")
      }
    } catch {
      case e: Exception =>
        println(s"处理日期 $currentDate 时出错: ${e.getMessage}")
        e.printStackTrace()
    }
  }
}