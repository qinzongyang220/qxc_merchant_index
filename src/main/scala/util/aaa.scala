...
object aaa {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 使用MyHive连接器连接Hive
    implicit val jobName: String = "aaa"
    ...
    try {
      ...
      val weekAgo = dateFormat.format(cal.getTime())
      println(s"分析区间: $weekAgo 到 $yesterday")
      processDateData(spark, weekAgo, yesterday)
    } catch {
      case e: Exception =>
  ...
  /**
   * 统计一周区间总数据，只保留stat_date字段，无date_range，只查曝光不为0的数据
   */
  private def processDateData(spark: SparkSession, startDate: String, endDate: String): Unit = {
    try {
    ...
    order_data AS (
      ...
      WHERE o.create_time BETWEEN '$startDate 00:00:00' AND '$endDate 23:59:59'
    ...
    pay_data AS (
      ...
      WHERE o.is_payed = 'true'
        AND o.pay_time BETWEEN '$startDate 00:00:00' AND '$endDate 23:59:59'
        AND oi.rec_time BETWEEN '$startDate 00:00:00' AND '$endDate 23:59:59'
      GROUP BY oi.prod_id, o.shop_id
    ),
    refund_single AS (
      ...
      WHERE r.apply_time BETWEEN '$startDate 00:00:00' AND '$endDate 23:59:59'
        AND r.refund_type = '2'
        AND oi.rec_time BETWEEN '$startDate 00:00:00' AND '$endDate 23:59:59'
    ),
    refund_order AS (
    ...
    WHERE p.status > '-1' AND COALESCE(pe.expose_count, 0) > 0
    ORDER BY expose DESC
    """
      val productAnalysisDF = spark.sql(productAnalysisSQL)
      val totalCount = productAnalysisDF.count()
      println(s"分析了 $totalCount 条商品数据")
      productAnalysisDF.show(Int.MaxValue, false)

      // 去重处理，避免唯一索引冲突
      val dedupDF = productAnalysisDF.dropDuplicates(Seq("prod_id", "shop_id", "stat_date", "status"))
      val dedupCount = dedupDF.count()
      println(s"去重后剩余 $dedupCount 条商品数据")

      // 检查是否还有重复（调试用）
      val dupCheck = dedupDF
        .groupBy("prod_id", "shop_id", "stat_date", "status")
        .count()
        .filter(col("count") > 1)

      if (dupCheck.count() > 0) {
        println("警告：去重后仍有重复数据！")
        dupCheck.show(false)
      }

      // 只保留曝光不为0的数据写入MySQL
      val finalDF = dedupDF.filter(col("expose") =!= 0)
      val finalCount = finalDF.count()
      println(s"最终曝光不为0的商品数: $finalCount")
      if (finalCount > 0) {
        writeToMySQL(finalDF, "tz_bd_merchant_product_analysis", endDate)
      }
    } catch {
      case e: Exception =>
        println(s"处理区间 $startDate ~ $endDate 时出错: ${e.getMessage}")
        e.printStackTrace()
    }
  }
}
